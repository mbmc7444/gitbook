...
각각의 triggers 에게 forEach 구문으로 mouseenter , mouseleave 이벤트를 만들어준다.

해당 triggers에 trigger-enter class를 추가\(이때 setTimemout\(\)으로 trigger-enter- active클래스를 0.15초 후에 추가한다.\)

getBoundingClientRect\(\)을 통해 이 각각의 dropdown의 위치를 확인한다. 

이제 dropdown의 사이즈에 따라 background의 사이즈가 변화하는데 같이 움직이게 하기위해 left, top width height값을 넣어준다.

```javascript
this.classList.contains('trigger-enter') && this.classList.add('trigger-enter-active')
// 해당 구문은 trigger-enter가 존재(true)해야만 trigger-enter-active를 추가하는 구문이다
```
