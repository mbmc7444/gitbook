...
보통 ajax 에서 많이 사용한다고 한다. 예를들어 input.value를 서버에서 가져오려고한다. input에 글자를 연달아 적을때 그 하나하나 값들을 전부 서버에 요청을 하게됩니다. 즉 느리고 ,비용적인 부분도 많이 들게된다 따라서 이러한 부분을 디바운싱하게되는데, 디바운싱 하게되면 value의 일정 시간동안 기다렸다가 마지막에 작성된 값만 서버에 요청하게된다.

**쓰로틀링**

마지막 함수가 호출된 후 일정 시간이 지나기 전에 다시 호출되지 않도록 하는 것

비슷한 개념으로 쓰로틀링이라는 개념이 있다. 컴퓨터 쓰로틀링으로 이해하면 편할거같다**.** 쓰로틀링은 보통 스크롤할때 많이 사용한다. 스크롤을 하게되면 수많은 숫자가 요청되게된다 그렇게되면 부하가 오게되고 브라우저가 느려지는 현상이 발생한다 그렇기때문에 스크롤을할때, 시간을 입력하여 일정 시간마다 이벤트가 발생 시킬수 있도록 코드를 작성한다.

```javascript
        const slideInAt = (window.scrollY + window.innerHeight) - sliderImage.height / 2;
        // 상단에서 이미지 중간까지의 거
        const imageBottom = sliderImage.offsetTop + sliderImage.height;
      // 이미지가 상단위치에 높이를 더하게되면 하단위치를 알수있다. 
         const isHalfShown = slideInAt > sliderImage.offsetTop;
 //  상단에서 이미지 중간까지 거리보다 이미지의 상단값이 커지면 즉 위에 보이면
        const isNotScrolledPast = window.scrollY < imageBottom;
        // 이미지의 바닥이 보이는지 확인하려는 의
```
